...
def main_dns():
    ...
    config_dns_file = "./config_onetier.yml" 
...
def dns_meta_analysis():
    ...
    siglist = np.array([3.0,3.0,1.0,0.5,0.25,0.0])
    alist = np.array([0.0] + [1.0]*5)
    T_phys_list = np.array([30.0,15.0,6.0,4.5])[[2,]] # different time horizons with which to calculate return period
    resolution_list = np.array([1,])
    expt_dir = join(scratch_dir, date_str, sub_date_str)
    forcing_dir_list = [join(expt_dir,(f"F6p0_K40_J0_a{alist[i]}_white_wave4mag{siglist[i]}").replace(".","p")) for i in [5,2,1]]
    ctrl_dir_list = [join(forcing_dir, "DNS") for forcing_dir in forcing_dir_list]
    first_ens = pickle.load(open(join(ctrl_dir_list[0],"output","ens"), "rb"))
